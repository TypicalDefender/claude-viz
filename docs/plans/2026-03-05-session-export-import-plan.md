...
func TestExportAll(t *testing.T) {
	...
	// Filename should start with "claude-viz-export-"
	if len(filename) < 14 || filename[:14] != "claude-viz-export-" {
		t.Errorf("filename = %q, want prefix %q", filename, "claude-viz-export-")
	}
...
func ExportAll(claudeDir string, metas []*model.SessionMeta, outputDir string) (string, error) {
	...
	filename := fmt.Sprintf("claude-viz-export-%s.zip", time.Now().Format("2006-01-02"))
	zipPath := filepath.Join(outputDir, filename)
